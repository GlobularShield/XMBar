...
namespace mdr
{
    ...
    MDRTask MDRHeadphones::RequestInitV2()
    {
        ...
        // Following are cached by the offical app based on the MAC address
        {
            ...
            if (mSupport.contains(MessageMdrV2FunctionType_Table1::GENERAL_SETTING_1))
            {
                ...
                SendCommandACK(t1::GsGetParam, {.type = t1::GsInquiredType::GENERAL_SETTING1});
            }
            if (mSupport.contains(MessageMdrV2FunctionType_Table1::GENERAL_SETTING_2))
            {
                ...
                SendCommandACK(t1::GsGetParam, {.type = t1::GsInquiredType::GENERAL_SETTING2});
            }
            if (mSupport.contains(MessageMdrV2FunctionType_Table1::GENERAL_SETTING_3))
            {
                ...
                SendCommandACK(t1::GsGetParam, {.type = t1::GsInquiredType::GENERAL_SETTING3});
            }
            if (mSupport.contains(MessageMdrV2FunctionType_Table1::GENERAL_SETTING_4))
            {
                ...
                SendCommandACK(t1::GsGetParam, {.type = t1::GsInquiredType::GENERAL_SETTING4});
            }

            /* DSEE */
            if (mSupport.contains(MessageMdrV2FunctionType_Table1::UPSCALING_AUTO_OFF))
                SendCommandACK(t1::AudioGetCapability, {.type = t1::AudioInquiredType::UPSCALING});
        }
        ...
        /* Playback Metadata */
        SendCommandACK(t1::GetPlayParam, {.type = t1::PlayInquiredType::PLAYBACK_CONTROL_WITH_CALL_VOLUME_ADJUSTMENT});

        /* Playback Volume */
        SendCommandACK(t1::GetPlayParam, {.type = t1::PlayInquiredType::MUSIC_VOLUME});

        /* Play/Pause */
        SendCommandACK(t1::GetPlayStatus, {.type = t1::PlayInquiredType::PLAYBACK_CONTROL_WITH_CALL_VOLUME_ADJUSTMENT});
                ...
                MessageMdrV2FunctionType_Table1::MODE_NC_ASM_NOISE_CANCELLING_DUAL_AMBIENT_SOUND_MODE_LEVEL_ADJUSTMENT))
        {
            SendCommandACK(t1::NcAsmGetParam,
                     ...
                     MessageMdrV2FunctionType_Table1::
        ...
        if (std::ranges::any_of(kPairingFunctions, [&](auto x) { return mSupport.contains(x); }))
        {
            /* Pairing Mode */
            SendCommandACK(
                t2::PeripheralGetStatus,
                {.type = t2::PeripheralInquiredType::PAIRING_DEVICE_MANAGEMENT_WITH_BLUETOOTH_CLASS_OF_DEVICE});

            /* Connected Devices */
            SendCommandACK(
                t2::PeripheralGetParam,
                {.type = t2::PeripheralInquiredType::PAIRING_DEVICE_MANAGEMENT_WITH_BLUETOOTH_CLASS_OF_DEVICE});
        }
        ...
        if (mSupport.contains(MessageMdrV2FunctionType_Table2::PAIRING_DEVICE_MANAGEMENT_CLASSIC_BT))
        {
            ...
            SendCommandACK(t2::PeripheralGetStatus,
                           {.type = t2::PeripheralInquiredType::PAIRING_DEVICE_MANAGEMENT_CLASSIC_BT});
        ...
        /* Connection Quality */
        if (mSupport.contains(MessageMdrV2FunctionType_Table1::CONNECTION_MODE_SOUND_QUALITY_CONNECTION_QUALITY))
        ...
        if (mProtocol.hasTable2)
        {
            // Enabled
            SendCommandACK(
                t2::VoiceGuidanceGetParam,
                {.type = t2::VoiceGuidanceInquiredType::MTK_TRANSFER_WO_DISCONNECTION_SUPPORT_LANGUAGE_SWITCH});
            // Volume
        ...
        constexpr UInt8 kLogSetStatusCommand[] = {static_cast<UInt8>(t1::Command::LOG_SET_STATUS), 0x01, 0x00};
    ...
    MDRTask MDRHeadphones::RequestCommitV2()
    {
        ...
        /* NC/ASM */
        if (mNcAsmAmbientLevel.dirty() || mNcAsmEnabled.dirty() || mNcAsmMode.dirty() || mNcAsmFocusOnVoice.dirty() ||
            mNcAsmAutoAsmEnabled.dirty() || mNcAsmNoiseAdaptiveSensitivity.dirty())
        {
            using namespace t1;
            if (mSupport.contains(
                    MessageMdrV2FunctionType_Table1::
        ...
        /* Connection Ops */
        {
            ...
            if (mSupport.contains(MessageMdrV2FunctionType_Table2::
                                      PAIRING_DEVICE_MANAGEMENT_WITH_BLUETOOTH_CLASS_OF_DEVICE_CLASSIC_BT) ||
                mSupport.contains(MessageMdrV2FunctionType_Table2::
                                      PAIRING_DEVICE_MANAGEMENT_WITH_BLUETOOTH_CLASS_OF_DEVICE_CLASSIC_LE))
            {
            ...
            if (mPairingMode.dirty())
            {
                ...
                res.base.type = type;
                res.btMode = mPairingMode.desired ? PeripheralBluetoothMode::INQUIRY_SCAN_MODE
        ...
        if (mSupport.contains(MessageMdrV2FunctionType_Table1::SMART_TALKING_MODE_TYPE2))
        {
            ...
            if (mSpeakToChatEnabled.dirty())
            {
                ...
                res.onOffValue = mSpeakToChatEnabled.desired ? MessageMdrV2EnableDisable::ENABLE
        ...
        if (mSupport.contains(MessageMdrV2FunctionType_Table1::LISTENING_OPTION))
        {
            ...
            if (mBGMModeEnabled.dirty() || mBGMModeRoomSize.dirty())
            {
                ...
                res.onOffSettingValue =
                    mBGMModeEnabled.desired ? MessageMdrV2EnableDisable::ENABLE : MessageMdrV2EnableDisable::DISABLE;
                ...
                MDR_LOG("S/W BGM BGM {} ROOM {} UPMIX {}", mBGMModeEnabled.desired, mBGMModeRoomSize.desired,
                        mUpmixCinemaEnabled.desired);
            }
            if (mUpmixCinemaEnabled.dirty())
            {
                AudioParamUpmixCinema res;
                res.base.command = Command::AUDIO_SET_PARAM;
                res.onOffSettingValue = mUpmixCinemaEnabled.desired ? MessageMdrV2EnableDisable::ENABLE
                                                                    : MessageMdrV2EnableDisable::DISABLE;
                SendCommandACK(AudioParamUpmixCinema, res);
                mUpmixCinemaEnabled.commit();
                MDR_LOG("S/W CNE BGM {} ROOM {} UPMIX {}", mBGMModeEnabled.desired, mBGMModeRoomSize.desired,
                        mUpmixCinemaEnabled.desired);
            }
        }
        ...
        if (mSupport.contains(MessageMdrV2FunctionType_Table1::HEAD_GESTURE_ON_OFF_TRAINING))
        {
            if (mHeadGestureEnabled.dirty())
            {
                ...
                res.settingValue = mHeadGestureEnabled.desired ? MessageMdrV2EnableDisable::ENABLE
                                                               : MessageMdrV2EnableDisable::DISABLE;
        ...
        if (mSupport.contains(MessageMdrV2FunctionType_Table1::PLAYBACK_CONTROL_BY_WEARING_REMOVING_HEADPHONE_ON_OFF))
        {
            using namespace t1;
            if (mAutoPauseEnabled.dirty())
            {
                ...
                res.settingValue =
                    mAutoPauseEnabled.desired ? MessageMdrV2EnableDisable::ENABLE : MessageMdrV2EnableDisable::DISABLE;
                SendCommandACK(SystemParamCommon, res);
        ...
        if (mVoiceGuidanceEnabled.dirty())
        {
            ...
            res.settingValue =
                mVoiceGuidanceEnabled.desired ? MessageMdrV2OnOffSettingValue::ON : MessageMdrV2OnOffSettingValue::OFF;
        ...
        /* Voice Guidance */
        if (mSupport.contains(
                MessageMdrV2FunctionType_Table2::
        ...
        if (mSafeListeningPreviewMode.dirty())
        {
            ...
            if (mSupport.contains(MessageMdrV2FunctionType_Table2::SAFE_LISTENING_HBS_1))
            {
                ...
                res.safeListeningMode = mSafeListeningPreviewMode.desired ? MessageMdrV2EnableDisable::ENABLE
                                                                          : MessageMdrV2EnableDisable::DISABLE;
